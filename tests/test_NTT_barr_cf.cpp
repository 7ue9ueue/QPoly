...
const uint32_t BARRETT_MU = 2309898375;
...
void ntt_SIMD(vi &a) {
    ...
    {
        int w1 = rt[1];
        v8i v_W = _mm256_castps_si256(_mm256_broadcast_ss((float*)&w1));
        for (int i = 0; i < n; i += 8) {
    ...
    // k = 2
    {
        int w0_val = rt[2];
        int w1_val = rt[3];
        v8i w0 = _mm256_castps_si256(_mm256_broadcast_ss((float*)&w0_val));
        v8i w1 = _mm256_castps_si256(_mm256_broadcast_ss((float*)&w1_val));
    ...
    // k = 4
    {
        v8i v_W = _mm256_loadu_si256((v8i*)&rt[4]); 
        v_W = _mm256_permute2x128_si256(v_W, v_W, 0x00);
...
bool test_correctness(int n, int num_tests = 10) {
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        // Test with reference implementation (using long long for verification)
        vll a_ref(input.begin(), input.end());
        ntt(a_ref);
        
        // Test with int reference
        vi a_ref_int = input;
        ntt_ref_int(a_ref_int);
        ...
        // Compare results
        for (int i = 0; i < n; i++) {
            if (a_ref[i] != a_simd[i] || a_ref_int[i] != a_simd[i]) {
                cout << "FAILED!\n";
                cout << "  Mismatch at index " << i << ": ";
                cout << "reference(ll) = " << a_ref[i] 
                     << ", reference(int) = " << a_ref_int[i]
                     << ", SIMD = " << a_simd[i] << "\n";
                return false;
            }
        }
...
void benchmark(int n, int iterations = 100) {
    ...
    // Warm up - ensure static twiddle factors are precomputed
    for (int i = 0; i < 10; i++) {
        vi a_ref = input;
        ntt_ref_int(a_ref);
        vi a_simd = input;
    ...
    cout << "  Reference (int): " << fixed << setprecision(3) << time_ref << " ms\n";
