...
void print (v8i x) {
    std::cerr << x[0] << " " << x[1] << " " << x[2] << " " << x[3] << std::endl;
}
...
alignas(32) uint32_t SMALL_ROOTS_MONT[8];
alignas(32) uint32_t SMALL_INV_ROOTS_MONT[8];
alignas(32) v8i wdv8i[32];

void init_twiddle_updates() {
    auto start = std::chrono::high_resolution_clock::now();
    int lg = 32;
    wd.assign(lg, 0); wrd.assign(lg, 0);
    for (int i = 0; i < lg - 1; i++) {
        uint32_t a = pow_mod(PRIM_ROOT, (MOD - 1) >> (i + 2));
        uint32_t b = pow_mod(PRIM_ROOT, ((MOD - 1) >> (i + 1)) * ((1LL << i) - 1));
        uint32_t f = mont_mul(to_mont(a), to_mont(inv_mod(b)));
        wd[i] = f;
        wrd[i] = to_mont(inv_mod(from_mont(f)));
    }
    for (int i = 0; i < lg - 2; i++) {
        uint32_t ww = mont_mul(wd[0], wd[i + 1]);
        wdv8i[i] = _mm256_setr_epi32(wd[i], 0, ww, 0, wd[i], 0, ww, 0);
    }
    uint32_t w_8 = pow_mod(PRIM_ROOT, (MOD - 1) >> 3);
    uint32_t iw_8 = inv_mod(w_8);
    
    uint32_t w_curr = 1;
    uint32_t iw_curr = 1;
    for(int i=0; i<8; ++i) {
        SMALL_ROOTS_MONT[i] = to_mont(w_curr);
        SMALL_INV_ROOTS_MONT[i] = to_mont(iw_curr);
        w_curr = (uint64_t)w_curr * w_8 % MOD;
        iw_curr = (uint64_t)iw_curr * iw_8 % MOD;
    }
...
// ============================================================================
v8i VT = _mm256_set1_epi32(2);
v8i VX = _mm256_set1_epi32(6);
void dif_ntt(v8i *f, const int &n, const uint32_t* rt) {    
    ...
    // Handle odd number of stages with a radix-2 pass first
    if (num_stages & 1) {
        int h = n >> 1;
        int h8 = h >> 3;
    ...
    for (; i >= 8; i >>= 2) {
        ...
        v8i* f2 = f + i8 * 2;
        v8i* f3 = f + i8 * 3;
        uint32_t r0 = rt[0], r1 = rt[1];
        uint32_t ir0 = rt[0] * M_INV, ir1 = rt[1] * M_INV;
        v8i rts = _mm256_setr_epi32(r0, 0, r0, 0, r0, 0, r1, 0); 
        for (int j = 0, k = 0; j < n8; j += i8 << 2, ++k) {
            //std::cerr << k << std::endl;
            v8i rts_INV = _mm256_mullo_epi32(rts, v_m); 
            v8i v_w0 = _mm256_shuffle_epi32(rts, 0b00'00'00'00);
            v8i v_w1 = _mm256_permutevar8x32_epi32(rts, VT);
            v8i v_w2 = _mm256_permutevar8x32_epi32(rts, VX);
            v8i v_w0_inv = _mm256_shuffle_epi32(rts_INV, 0b00'00'00'00);
            v8i v_w1_inv = _mm256_permutevar8x32_epi32(rts_INV, VT);
            v8i v_w2_inv = _mm256_permutevar8x32_epi32(rts_INV, VX);
            ...
            rts = _mm256_mont_mul(rts, wdv8i[__builtin_ctz(~k)]);
        }
    }
...
void dit_ntt(v8i *f, const int &n, const uint32_t* irt) {
    ...
    // Handle odd number of stages with a final radix-2 pass
