...
void dif_ntt(v8i *f, const int &n, const uint32_t* rt) {
    ...
    for (; i >= 8; i >>= 2) {
        ...
        for (int j = 0, k = 0; j < n8; j += i8 << 2, ++k) {
            v8i v_w0 = _mm256_set1_epi32(rt[k]);
            v8i v_w1 = _mm256_set1_epi32(rt[(k << 1)]);
            v8i v_w2 = _mm256_set1_epi32(rt[(k << 1) + 1]);
            v8i v_w0_inv = _mm256_mul_epu32(v_w0, v_m);
            v8i v_w1_inv = _mm256_mul_epu32(v_w1, v_m);
            v8i v_w2_inv = _mm256_mul_epu32(v_w2, v_m);
...
void dit_ntt(v8i *f, const int &n, const uint32_t* irt) {
    ...
    for (; i << 2 <= n; i <<= 2) {
        int i8 = i >> 3; 
        for (int j = 0, k = 0; j < n8; j += i8 << 2, ++k) {
            v8i v_iw0 = _mm256_set1_epi32(irt[k]);
            v8i v_iw1 = _mm256_set1_epi32(irt[k << 1]);
            v8i v_iw2 = _mm256_set1_epi32(irt[(k << 1) + 1]);
            v8i v_iw0_inv = _mm256_mul_epu32(v_iw0, v_m);
            v8i v_iw1_inv = _mm256_mul_epu32(v_iw1, v_m);
            v8i v_iw2_inv = _mm256_mul_epu32(v_iw2, v_m);
            v8i* __restrict f0 = f + j;
            v8i* __restrict f1 = f + j + i8;
            v8i* __restrict f2 = f + j + i8 * 2;
            v8i* __restrict f3 = f + j + i8 * 3;
