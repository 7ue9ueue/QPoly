...
// Insert the entire code from the document here
// (Montgomery, Montgomery_simd, NTT classes)
...
u32* alloc_aligned(int size) {
    void* ptr = nullptr;
    posix_memalign(&ptr, 32, size * sizeof(u32));
    return (u32*)ptr;
}

int main() {
    // Use a good NTT-friendly prime: 998244353 = 119 * 2^23 + 1
    ...
    u32* a = alloc_aligned(N);
    u32* b = alloc_aligned(N);
    u32* c = alloc_aligned(N);
    ...
    timer.start();
    ntt.transform_forward(LG, a);
    ntt.transform_forward(LG, b);
    long long dif_time = timer.elapsed_ns();
    
    // Time pointwise multiply
    timer.start();
    ntt.aux_dot_mod<false>(LG, a, b, c);
    long long mul_time = timer.elapsed_ns();
    
    // Time DIT (inverse transform) - without final scaling for benchmark
    timer.start();
    ntt.transform_inverse(LG, c);
    ...
    std::cout << "Total     " << std::setw(10) << (root_time + dif_time + mul_time + dit_time) / 1000.0 / 1000.0 << " ms\n";
    // Cleanup
    free(a);
    free(b);
    free(c);
