// added blocking and prefetching. 
...
constexpr int BLOCK = 2048; // Blocking factor
...
// ============================================================================
// DIF NTT (Mixed Radix-2/4 with Blocking)
// ============================================================================
void dif_ntt(uint32_t *f, const int &n) {
    ...
    if (num_stages & 1) {
        ...
        for (int j = 0, k = 0; j != n; j += h << 1, ++k) {
            ...
            // Blocking
            for (int block_start = j; block_start < j + h; block_start += BLOCK) {
                int block_end = std::min(block_start + BLOCK, j + h);
                for (int p = block_start; p < block_end; p += 8) {
    ...
    for (; i >= 8; i >>= 2) {
        for (int j = 0, k = 0; j != n; j += i << 2, ++k) {
            ...
            // Blocking: process BLOCK elements at a time
            for (int block_start = j; block_start < j + i; block_start += BLOCK) {
                int block_end = std::min(block_start + BLOCK, j + i);
                __builtin_prefetch(f + block_start + BLOCK);
                __builtin_prefetch(f + block_start + BLOCK + i);
                __builtin_prefetch(f + block_start + BLOCK + 2*i);
                __builtin_prefetch(f + block_start + BLOCK + 3*i);
                for (int p = block_start; p < block_end; p += 8) {
    ...
    // Inner loops for i = 4, 2, 1 with blocking
    for (int block_start = 0; block_start < n; block_start += BLOCK) {
        int block_end = std::min(block_start + BLOCK, n);
        
        // i = 4
        for (int j = block_start, k = block_start >> 3; j < block_end; j += 8, ++k) {
    ...
    for (int block_start = 0; block_start < n; block_start += BLOCK) {
        ...
        // i = 2
        for (int j = block_start; j < block_end; j += 8) {
...
// ============================================================================
// DIT NTT (Mixed Radix-2/4 with Blocking)
// ============================================================================
void dit_ntt(uint32_t *f, const int &n) {
    ...
    // Inner loops for i = 1, 2, 4 with blocking
    for (int block_start = 0; block_start < n; block_start += BLOCK) {
        int block_end = std::min(block_start + BLOCK, n);
    ...
    for (int block_start = 0; block_start < n; block_start += BLOCK) {
        int block_end = std::min(block_start + BLOCK, n);
        for (int i = block_start; i < block_end; i += 8) {
...
void run_test_logic(int L) {
    ...
    for (int block_start = 0; block_start < L; block_start += BLOCK) {
        int block_end = std::min(block_start + BLOCK, L);
        for (int i = block_start; i < block_end; i += 8) {
...
Root precomputation:         29889 ns
DIF nested loops:            18015 ns
DIF small loops:             17240 ns
Pointwise multiply:           1935 ns
DIT small loops:              8382 ns
DIT nested loops:            10939 ns
Total:                       86400 ns
========================================
...
Root precomputation:         40013 ns
DIF nested loops:            41300 ns
DIF small loops:             34224 ns
Pointwise multiply:           3445 ns
DIT small loops:             16692 ns
DIT nested loops:            22750 ns
Total:                      158424 ns
========================================
...
Root precomputation:         63614 ns
DIF nested loops:            93850 ns
DIF small loops:             69489 ns
Pointwise multiply:           6854 ns
DIT small loops:             43932 ns
DIT nested loops:            48982 ns
Total:                      326721 ns
========================================
...
Root precomputation:         96887 ns
DIF nested loops:           201286 ns
DIF small loops:            134129 ns
Pointwise multiply:          13751 ns
DIT small loops:             65307 ns
DIT nested loops:           102337 ns
Total:                      613697 ns
========================================
...
Root precomputation:        177575 ns
DIF nested loops:           459196 ns
DIF small loops:            290323 ns
Pointwise multiply:          29695 ns
DIT small loops:            135887 ns
DIT nested loops:           231272 ns
Total:                     1323948 ns
========================================
...
Root precomputation:        329075 ns
DIF nested loops:          1019542 ns
DIF small loops:            572695 ns
Pointwise multiply:          56335 ns
DIT small loops:            279733 ns
DIT nested loops:           497709 ns
Total:                     2755089 ns
========================================
...
Root precomputation:        628005 ns
DIF nested loops:          2177900 ns
DIF small loops:           1145737 ns
Pointwise multiply:         124091 ns
DIT small loops:            543295 ns
DIT nested loops:          1067523 ns
Total:                     5686551 ns
========================================
...
Root precomputation:       1502127 ns
DIF nested loops:          5134212 ns
DIF small loops:           2429209 ns
Pointwise multiply:         213530 ns
DIT small loops:           1189284 ns
DIT nested loops:          2632503 ns
Total:                    13100865 ns
========================================
...
Root precomputation:       2997286 ns
DIF nested loops:         10901616 ns
DIF small loops:           4775574 ns
Pointwise multiply:         517829 ns
DIT small loops:           2374290 ns
DIT nested loops:          5399945 ns
Total:                    26966540 ns
========================================
...
Root precomputation:       5672701 ns
DIF nested loops:         22423592 ns
DIF small loops:           9204200 ns
Pointwise multiply:        1238686 ns
DIT small loops:           4525371 ns
DIT nested loops:         10980736 ns
Total:                    54045286 ns
========================================
...
Root precomputation:      11076369 ns
DIF nested loops:         49258369 ns
DIF small loops:          18101647 ns
Pointwise multiply:        2565194 ns
DIT small loops:           9219665 ns
DIT nested loops:         24042989 ns
Total:                   114264233 ns
========================================
