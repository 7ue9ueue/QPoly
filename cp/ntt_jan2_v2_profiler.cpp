...
const uint32_t IMAG_UNIT = 911660635; // 4th root of unity (3^((MOD-1)/4))
const v8i v_imag = _mm256_set1_epi32(IMAG_UNIT);
const v8i v_imag_inv = _mm256_set1_epi32(MOD - IMAG_UNIT); // Inverse is -i
...
void dif_ntt(v8i *f, const int &n) {
    ...
    // Radix-4 passes: Flattened Logic
    // i is the quarter-block size in u32, i8 is in v8i
    for (; i >= 8; i >>= 2) {
        int i8 = i >> 3; 
        for (int j = 0, k = 0; j < n8; j += i8 << 2, ++k) {
            // rt[k]      -> W^{2k} (Middle/Half twist)
            // rt[2k]     -> W^{k}  (Quarter twist 1)
            // rt[2k+1]   -> W^{3k} (Quarter twist 3)
            const v8i v_w2k = _mm256_set1_epi32(rt[k]);      
            const v8i v_wk  = _mm256_set1_epi32(rt[2*k]);    
            const v8i v_w3k = _mm256_set1_epi32(rt[2*k + 1]); 
            ...
            for (int p = 0; p < i8; ++p) {
                ...
                // Radix-4 Butterfly Step 1 (Sums/Diffs)
                v8i t3 = _mm256_sub_mod(a1, a3);
                v8i t1 = _mm256_add_mod(a1, a3);
                v8i t0 = _mm256_add_mod(a0, a2);
                v8i t2 = _mm256_sub_mod(a0, a2);

                // Internal Imaginary Twist (Multiplication 1)
                // t3_i = (a1 - a3) * i
                v8i t3_i = _mm256_mont_mul(t3, v_imag);

                // Recombination
                v8i r0 = _mm256_add_mod(t0, t1); // Out 0
                v8i r1 = _mm256_sub_mod(t0, t1); // Out 2 (needs W^2k)
                v8i r2 = _mm256_add_mod(t2, t3_i); // Out 1 (needs W^k)
                v8i r3 = _mm256_sub_mod(t2, t3_i); // Out 3 (needs W^3k)

                // External Twiddle Multiplications (Multiplications 2, 3, 4)
                f0[p] = r0;
                f2[p] = _mm256_mont_mul(r1, v_w2k);
                f1[p] = _mm256_mont_mul(r2, v_wk);
                f3[p] = _mm256_mont_mul(r3, v_w3k);
            }
        }
    }

    // Small loops remain unchanged (they are already highly specialized)
...
void dit_ntt(v8i *f, const int &n) {
    ...
    for (; i << 2 <= n; i <<= 2) {
        int i8 = i >> 3;
        for (int j = 0, k = 0; j < n8; j += i8 << 2, ++k) {
            const v8i v_iw2k = _mm256_set1_epi32(irt[k]);
            const v8i v_iwk  = _mm256_set1_epi32(irt[2*k]);
            const v8i v_iw3k = _mm256_set1_epi32(irt[2*k + 1]);
            ...
            for (int p = 0; p < i8; ++p) {
                ...
                // Apply Twiddles First (DIT)
                // a0 is untwisted (mul 1)
                // a1 twisted by W^k (here W^-k) -> Multiplicaton 1
                // a2 twisted by W^2k -> Multiplication 2
                // a3 twisted by W^3k -> Multiplication 3
                
                v8i a1_tw = _mm256_mont_mul(a1, v_iwk);
                v8i a2_tw = _mm256_mont_mul(a2, v_iw2k);
                v8i a3_tw = _mm256_mont_mul(a3, v_iw3k);

                // Butterfly 
                v8i t0 = _mm256_add_mod(a0, a2_tw);
                v8i t1 = _mm256_sub_mod(a0, a2_tw);
                v8i t2 = _mm256_add_mod(a1_tw, a3_tw);
                v8i t3 = _mm256_sub_mod(a1_tw, a3_tw);

                // Imaginary Twist (Multiplication 4)
                // In DIT (Inverse), we multiply by -i
                v8i t3_i = _mm256_mont_mul(t3, v_imag_inv);

                // Final Recombine
                f0[p] = _mm256_add_mod(t0, t2);
                f1[p] = _mm256_add_mod(t1, t3_i);
                f2[p] = _mm256_sub_mod(t0, t2);
                f3[p] = _mm256_sub_mod(t1, t3_i);
            }
        }
    }
