...
void dif_ntt(uint32_t *f, const int &n) {
    ...
    // DIF small loops - MERGED i = 4, 2, 1 with single load/store
    // Reordered for better ILP: loads first, independent ops together
    auto start_small = std::chrono::high_resolution_clock::now();
    
    for (int j = 0, k = 0; j != n; j += 8, ++k) {
        // === Hoist all memory loads ===
        v8i v_f = _mm256_loadu_si256((v8i*)(f + j));
        __m128i rt2_raw = _mm_loadl_epi64((__m128i*)(rt + (k << 1)));
        __m128i rt1_raw = _mm_loadu_si128((__m128i*)(rt + (k << 2)));
        
        // === Prepare all twiddle factors (independent, can overlap with load latency) ===
        v8i v_rt4 = _mm256_set1_epi32(rt[k]);
        v8i v_rt2 = _mm256_permutevar8x32_epi32(_mm256_castsi128_si256(rt2_raw), perm_i2);
        v8i v_rt1 = _mm256_permutevar8x32_epi32(_mm256_castsi128_si256(rt1_raw), perm_i1);
        
        // === Stage i = 4 ===
        v8i v_u4 = _mm256_permute2x128_si256(v_f, v_f, 0x00);   // independent
        v8i v_q4 = _mm256_permute2x128_si256(v_f, v_f, 0x11);   // independent
        v8i v_v4 = _mm256_mont_mul(v_q4, v_rt4);                // high latency, start early
        v8i v_sum4 = _mm256_add_mod(v_u4, v_v4);                // independent
        v8i v_diff4 = _mm256_sub_mod(v_u4, v_v4);               // independent
        v_f = _mm256_permute2x128_si256(v_sum4, v_diff4, 0x20);
        
        // === Stage i = 2 ===
        v8i v_u2 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(1, 0, 1, 0));  // independent
        v8i v_q2 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(3, 2, 3, 2));  // independent
        v8i v_v2 = _mm256_mont_mul(v_q2, v_rt2);
        v8i v_sum2 = _mm256_add_mod(v_u2, v_v2);                        // independent
        v8i v_diff2 = _mm256_sub_mod(v_u2, v_v2);                       // independent
        v_f = _mm256_unpacklo_epi64(v_sum2, v_diff2);
        
        // === Stage i = 1 ===
        v8i v_u1 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(2, 2, 0, 0));  // independent
        v8i v_q1 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(3, 3, 1, 1));  // independent
        v8i v_v1 = _mm256_mont_mul(v_q1, v_rt1);
        v8i v_sum1 = _mm256_add_mod(v_u1, v_v1);                        // independent
        v8i v_diff1 = _mm256_sub_mod(v_u1, v_v1);                       // independent
        v8i v_diff1_shuf = _mm256_shuffle_epi32(v_diff1, _MM_SHUFFLE(2, 3, 0, 1));
        v_f = _mm256_blend_epi32(v_sum1, v_diff1_shuf, 0xAA);
        
        // Single store
        _mm256_storeu_si256((v8i*)(f + j), v_f);
    }
...
void dit_ntt(uint32_t *f, const int &n) {
    ...
    // DIT small loops - MERGED i = 1, 2, 4 with single load/store
    // Reordered for better ILP: loads first, independent ops together
    auto start_small = std::chrono::high_resolution_clock::now();
    ...
    for (int j = 0, k = 0; j != n; j += 8, ++k) {
        // === Hoist all memory loads ===
        v8i v_f = _mm256_loadu_si256((v8i*)(f + j));
        __m128i irt2_raw = _mm_loadl_epi64((__m128i*)(irt + (k << 1)));
        __m128i irt1_raw = _mm_loadu_si128((__m128i*)(irt + (k << 2)));
        
        // === Prepare all twiddle factors (independent, can overlap with load latency) ===
        v8i v_irt4 = _mm256_set1_epi32(irt[k]);
        v8i v_irt2 = _mm256_permutevar8x32_epi32(_mm256_castsi128_si256(irt2_raw), perm_i2);
        v8i v_irt1 = _mm256_permutevar8x32_epi32(_mm256_castsi128_si256(irt1_raw), perm_i1);
        
        // === Stage i = 1 ===
        v8i v_u1 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(2, 2, 0, 0));  // independent
        v8i v_v1 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(3, 3, 1, 1));  // independent
        v8i v_sum1 = _mm256_add_mod(v_u1, v_v1);                        // independent
        v8i v_sub1 = _mm256_sub_mod(v_u1, v_v1);                        // independent
        v8i v_diff1 = _mm256_mont_mul(v_sub1, v_irt1);
        v8i v_diff1_shuf = _mm256_shuffle_epi32(v_diff1, _MM_SHUFFLE(2, 3, 0, 1));
        v_f = _mm256_blend_epi32(v_sum1, v_diff1_shuf, 0xAA);
        
        // === Stage i = 2 ===
        v8i v_u2 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(1, 0, 1, 0));  // independent
        v8i v_v2 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(3, 2, 3, 2));  // independent
        v8i v_sum2 = _mm256_add_mod(v_u2, v_v2);                        // independent
        v8i v_sub2 = _mm256_sub_mod(v_u2, v_v2);                        // independent
        v8i v_diff2 = _mm256_mont_mul(v_sub2, v_irt2);
        v_f = _mm256_unpacklo_epi64(v_sum2, v_diff2);
        
        // === Stage i = 4 ===
        v8i v_u4 = _mm256_permute2x128_si256(v_f, v_f, 0x00);           // independent
        v8i v_v4 = _mm256_permute2x128_si256(v_f, v_f, 0x11);           // independent
        v8i v_sum4 = _mm256_add_mod(v_u4, v_v4);                        // independent
        v8i v_sub4 = _mm256_sub_mod(v_u4, v_v4);                        // independent
        v8i v_diff4 = _mm256_mont_mul(v_sub4, v_irt4);
        v_f = _mm256_permute2x128_si256(v_sum4, v_diff4, 0x20);
...
void run_test_logic(int L) {
    ...
    // Pointwise multiplication timing
