//alternate between radix 2 and radix 4
...
// DIF Radix-2 butterfly: operates on two blocks f0[0..len-1] and f1[0..len-1]
// f0[i] = f0[i] + w * f1[i]
// f1[i] = f0[i] - w * f1[i]
[[gnu::always_inline]] 
inline void butterfly_dif_radix2(v8i* f0, v8i* f1, int len8, uint32_t w) {
    v8i v_w = _mm256_set1_epi32(w);
    v8i v_w_inv = _mm256_set1_epi32(w * M_INV);
...
// DIF Radix-4 butterfly: operates on four blocks f0, f1, f2, f3, each of length len8
// w0 is the main twiddle factor, w1 and w2 are derived twiddles
// Computes the radix-4 DIF butterfly transformation
[[gnu::always_inline]] 
inline void butterfly_dif_radix4(v8i* f0, v8i* f1, v8i* f2, v8i* f3, int len8,
                                  uint32_t w0, uint32_t w1, uint32_t w2) {
    v8i v_w0 = _mm256_set1_epi32(w0);
    v8i v_w1 = _mm256_set1_epi32(w1);
    v8i v_w2 = _mm256_set1_epi32(w2);
    v8i v_w0_inv = _mm256_set1_epi32(w0 * M_INV);
    v8i v_w1_inv = _mm256_set1_epi32(w1 * M_INV);
    v8i v_w2_inv = _mm256_set1_epi32(w2 * M_INV);
    for (int p = 0; p < len8; ++p) {
        v8i a3 = f3[p];
        v8i a2 = f2[p];
        v8i a1 = f1[p];
...
// DIF small butterfly for size 4 (within v8i, 128-bit lanes)
[[gnu::always_inline]]
...
// DIF small butterfly for size 2 (within 64-bit lanes)
[[gnu::always_inline]]
...
// DIF small butterfly for size 1 (within 32-bit elements)
[[gnu::always_inline]]
...
// DIT Radix-2 butterfly: operates on two blocks f0[0..len-1] and f1[0..len-1]
// temp = f0[i] - f1[i]
// f0[i] = f0[i] + f1[i]
// f1[i] = temp * w
[[gnu::always_inline]]
inline void butterfly_dit_radix2(v8i* f0, v8i* f1, int len8, uint32_t w) {
    v8i v_w = _mm256_set1_epi32(w);
    v8i v_w_inv = _mm256_set1_epi32(w * M_INV);
    for (int p = 0; p < len8; ++p) {
        v8i v_u = f0[p];
...
// DIT Radix-4 butterfly: operates on four blocks f0, f1, f2, f3, each of length len8
// Computes the radix-4 DIT butterfly transformation (inverse of DIF radix-4)
[[gnu::always_inline]]
inline void butterfly_dit_radix4(v8i* f0, v8i* f1, v8i* f2, v8i* f3, int len8,
                                  uint32_t iw0, uint32_t iw1, uint32_t iw2) {
    v8i v_iw0 = _mm256_set1_epi32(iw0);
    v8i v_iw1 = _mm256_set1_epi32(iw1);
    v8i v_iw2 = _mm256_set1_epi32(iw2);
    v8i v_iw0_inv = _mm256_set1_epi32(iw0 * M_INV);
    v8i v_iw1_inv = _mm256_set1_epi32(iw1 * M_INV);
    v8i v_iw2_inv = _mm256_set1_epi32(iw2 * M_INV);
    for (int p = 0; p < len8; ++p) {
        v8i a0 = f0[p];
        v8i a1 = f1[p];
        v8i a2 = f2[p];
...
// DIT small butterfly for size 1 (within 32-bit elements)
[[gnu::always_inline]]
...
// DIT small butterfly for size 2 (within 64-bit lanes)
[[gnu::always_inline]]
...
// DIT small butterfly for size 4 (within v8i, 128-bit lanes)
[[gnu::always_inline]]
...
// ============================================================================
// DIF NTT (using alternating radix-2 and radix-4 butterflies)
// ============================================================================
void dif_ntt(v8i *f, const int &n, const uint32_t* rt) {    
    ...
    int num_stages = log_n - 3;  // Outer stages (last 3 handled by small butterflies)
    
    // block_size tracks current block size (in elements)
    // Operations split blocks: radix-2 halves, radix-4 quarters
    int block_size = n;
    int stages_done = 0;
    
    // Alternate: radix-2 (1 stage), radix-4 (2 stages), radix-2, radix-4, ...
    while (stages_done < num_stages) {
        // Try radix-2 (1 stage)
        if (stages_done < num_stages && block_size >= 16) {
            int half = block_size >> 1;  // half size = block_size / 2
            int h8 = half >> 3;
            for (int j = 0, k = 0; j < n8; j += h8 << 1, ++k) {
                butterfly_dif_radix2(f + j, f + j + h8, h8, rt[k]);
            }
            block_size = half;  // New block size after radix-2
            stages_done += 1;
        }
        
        // Try radix-4 (2 stages)
        if (stages_done < num_stages && block_size >= 32) {
            int i = block_size >> 2;  // quarter size = block_size / 4
            int i8 = i >> 3;
            int inc = i8 << 2;
            v8i* f0 = f;
            v8i* f1 = f + i8;
            ...
            for (int j = 0, k = 0; j < n8; j += i8 << 2, ++k) {
                butterfly_dif_radix4(f0, f1, f2, f3, i8, rt[k], rt[k << 1], rt[(k << 1) + 1]);
                f0 += inc;
                f1 += inc;
                f2 += inc;
                f3 += inc;
            }
            block_size = i;  // New block size after radix-4
            stages_done += 2;
        } else if (stages_done < num_stages) {
            // Can't do radix-4 (block too small), do radix-2 instead
            if (block_size >= 16) {
                ...
                break;  // Block too small for outer butterflies
            }
        }
    }
    ...
    // Small butterflies for final 3 stages (within v8i)
...
// ============================================================================
// DIT NTT (using alternating radix-4 and radix-2 butterflies)
// ============================================================================
void dit_ntt(v8i *f, const int &n, const uint32_t* irt) {
    ...
    // Small butterflies for first 3 stages (within v8i)
    ...
    // block_size tracks current block size (in elements)
    // After small butterflies, block_size = 8
    // Operations combine blocks: radix-4 quadruples, radix-2 doubles
    int block_size = 8;
    int stages_done = 0;
    
    // Alternate: radix-4 (2 stages), radix-2 (1 stage), radix-4, radix-2, ...
    while (stages_done < num_outer_stages) {
        // Try radix-4 (2 stages): combines 4 blocks of size block_size
        if (stages_done + 2 <= num_outer_stages && block_size * 4 <= n) {
            int i = block_size;  // quarter size = current block_size
            int i8 = i >> 3;
            int inc = i8 << 2;
            v8i* f0 = f;
            v8i* f1 = f + i8;
            ...
            v8i* f3 = f + i8 * 3;
            for (int j = 0, k = 0; j < n8; j += i8 << 2, ++k) {
                butterfly_dit_radix4(f0, f1, f2, f3, i8, irt[k], irt[k << 1], irt[(k << 1) + 1]);
                f0 += inc;
                f1 += inc;
                f2 += inc;
                f3 += inc;
            }
            block_size <<= 2;  // block_size *= 4 after radix-4
            stages_done += 2;
        }
        
        // Try radix-2 (1 stage): combines pairs of blocks of size block_size
        if (stages_done < num_outer_stages && block_size * 2 <= n) {
            int half = block_size;  // half size = current block_size
            int h8 = half >> 3;
            for (int j = 0, k = 0; j < n8; j += h8 << 1, ++k) {
                butterfly_dit_radix2(f + j, f + j + h8, h8, irt[k]);
            }
            block_size <<= 1;  // block_size *= 2 after radix-2
            stages_done += 1;
        } 
        else if (stages_done < num_outer_stages) {
            // Need more stages but can't do radix-2, try radix-4 with remaining
            if (stages_done + 2 <= num_outer_stages && block_size * 4 <= n) {
                int i = block_size;
                int i8 = i >> 3;
                int inc = i8 << 2;
    ...
    for (int j = 0; j < n8; ++j) {
        f[j] = _mm256_mod(_mm256_mont_mul_fixed(f[j], v_inv_n, v_inv_n_inv));
    }
...
int main() {
    ...
    // Using aligned_alloc for proper AVX2 alignment
    // ------------------------------------------------------------------------
    ...
    run_benchmark(A.get(), B.get(), A_bak.get(), B_bak.get(), roots.get(), inv_roots.get(), root_size);
