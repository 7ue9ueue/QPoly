...
struct TimingStats {
    ...
    void print(const std::string& label, int divisor = 1) const {
        ...
        std::cout << "DIF small loops (Fused):" << std::setw(10) << (dif_small_loops_ns / divisor) << " ns\n";
        std::cout << "Pointwise multiply:     " << std::setw(10) << (pointwise_mult_ns / divisor) << " ns\n";
        std::cout << "DIT small loops (Fused):" << std::setw(10) << (dit_small_loops_ns / divisor) << " ns\n";
...
// ============================================================================
// FUSED LAYERS (Explicit Formulas)
// ============================================================================

// Fuses the last 3 layers of DIF (sizes 4, 2, 1) inside the register.
// Applies Cooley-Tukey butterflies: U + V*W, U - V*W
inline void dif_last_3_layers(v8i* f, const int n8, const uint32_t* rt) {
    const v8i p2_perm = _mm256_setr_epi32(0, 0, 0, 0, 1, 1, 1, 1);
    const v8i p1_perm = _mm256_setr_epi32(0, 0, 1, 1, 2, 2, 3, 3);

    for (int j = 0; j < n8; ++j) {
        v8i v_f = f[j];

        v8i w1 = _mm256_set1_epi32(rt[j]);
        v8i u1 = _mm256_permute2x128_si256(v_f, v_f, 0x00);
        v8i q1 = _mm256_permute2x128_si256(v_f, v_f, 0x11);
        v8i v1 = _mm256_mont_mul(q1, w1);
        v8i v_f2 = _mm256_permute2x128_si256(_mm256_add_mod(u1, v1), _mm256_sub_mod(u1, v1), 0x20);
        v8i w2_raw = _mm256_castsi128_si256(_mm_loadl_epi64((__m128i*)(rt + (j << 1))));
        v8i w2 = _mm256_permutevar8x32_epi32(w2_raw, p2_perm);
        
        v8i u2 = _mm256_shuffle_epi32(v_f2, _MM_SHUFFLE(1, 0, 1, 0));
        v8i q2 = _mm256_shuffle_epi32(v_f2, _MM_SHUFFLE(3, 2, 3, 2));
        v8i v2_mont = _mm256_mont_mul(q2, w2);
        v8i v_f3 = _mm256_unpacklo_epi64(_mm256_add_mod(u2, v2_mont), _mm256_sub_mod(u2, v2_mont));

        v8i w4_raw = _mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(rt + (j << 2))));
        v8i w4 = _mm256_permutevar8x32_epi32(w4_raw, p1_perm);

        v8i u3 = _mm256_shuffle_epi32(v_f3, _MM_SHUFFLE(2, 2, 0, 0));
        v8i q3 = _mm256_shuffle_epi32(v_f3, _MM_SHUFFLE(3, 3, 1, 1));
        v8i v3 = _mm256_mont_mul(q3, w4);
        v8i np = _mm256_add_mod(u3, v3);
        v8i nq = _mm256_sub_mod(u3, v3);

        f[j] = _mm256_blend_epi32(np, _mm256_shuffle_epi32(nq, _MM_SHUFFLE(2, 3, 0, 1)), 0xAA);
    }
}

inline void dit_first_3_layers(v8i* f, const int n8, const uint32_t* irt) {
    const v8i p2_perm = _mm256_setr_epi32(0, 0, 0, 0, 1, 1, 1, 1);
    const v8i p1_perm = _mm256_setr_epi32(0, 0, 1, 1, 2, 2, 3, 3);

    for (int j = 0; j < n8; ++j) {
        v8i v_f = f[j];
        v8i w1_raw = _mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(irt + (j << 2))));
        v8i w1 = _mm256_permutevar8x32_epi32(w1_raw, p1_perm);
        
        v8i u1 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(2, 2, 0, 0));
        v8i v1 = _mm256_shuffle_epi32(v_f, _MM_SHUFFLE(3, 3, 1, 1));
        v8i sum1 = _mm256_add_mod(u1, v1);
        v8i diff1 = _mm256_mont_mul(_mm256_sub_mod(u1, v1), w1);
        v8i v_f2 = _mm256_blend_epi32(sum1, _mm256_shuffle_epi32(diff1, _MM_SHUFFLE(2, 3, 0, 1)), 0xAA);

        v8i w2_raw = _mm256_castsi128_si256(_mm_loadl_epi64((__m128i*)(irt + (j << 1))));
        v8i w2 = _mm256_permutevar8x32_epi32(w2_raw, p2_perm);

        v8i u2 = _mm256_shuffle_epi32(v_f2, _MM_SHUFFLE(1, 0, 1, 0));
        v8i v2 = _mm256_shuffle_epi32(v_f2, _MM_SHUFFLE(3, 2, 3, 2));
        v8i v_f3 = _mm256_unpacklo_epi64(_mm256_add_mod(u2, v2), _mm256_mont_mul(_mm256_sub_mod(u2, v2), w2));


        v8i w4 = _mm256_set1_epi32(irt[j]);
        v8i u3 = _mm256_permute2x128_si256(v_f3, v_f3, 0x00);
        v8i v3 = _mm256_permute2x128_si256(v_f3, v_f3, 0x11);
        
        f[j] = _mm256_permute2x128_si256(_mm256_add_mod(u3, v3), _mm256_mont_mul(_mm256_sub_mod(u3, v3), w4), 0x20);
    }
}
...
void dif_ntt(v8i *f, const int &n) {
    ...
    // --- Start Small Loop Timing (FUSED) ---
    auto start_small = std::chrono::high_resolution_clock::now();

    // Call the fused function for layers 4, 2, 1
    dif_last_3_layers(f, n8, rt);
...
void dit_ntt(v8i *f, const int &n) {
    ...
    // Call the fused function for layers 1, 2, 4
    dit_first_3_layers(f, n8, irt);
