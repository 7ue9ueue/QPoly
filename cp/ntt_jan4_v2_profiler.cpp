...
v8i _mm256_mont_mul_pointwise(v8i a, v8i b) {
    v8i a_sh = _mm256_bsrli_epi128(a, 4);
...
// Moved constants to top for global visibility (needed for allocation sizes)
const int maxn = 1 << 22; 
const int maxn8 = maxn >> 3;
...
// Global vectors removed, now using stack arrays passed by pointer

void reset_roots(uint32_t* roots, uint32_t* inv_roots, int& size) {
    roots[0] = to_mont(1);
    inv_roots[0] = to_mont(1);
    size = 1;
}

void get_root_mont(int n, uint32_t* roots, uint32_t* inv_roots, int& size) {
    auto start = std::chrono::high_resolution_clock::now();
...
// DIF NTT (Mixed Radix-2/4) - v8i array version
// ============================================================================
void dif_ntt(v8i *f, const int &n, const uint32_t* roots) {
    const uint32_t* rt = roots;
    const v8i perm_i2 = _mm256_setr_epi32(0, 0, 0, 0, 1, 1, 1, 1);
    const v8i perm_i1 = _mm256_setr_epi32(0, 0, 1, 1, 2, 2, 3, 3);
...
// DIT NTT (Mixed Radix-2/4) - v8i array version
// ============================================================================
void dit_ntt(v8i *f, const int &n, const uint32_t* inv_roots) {
    const uint32_t* irt = inv_roots;
    const v8i perm_i2 = _mm256_setr_epi32(0, 0, 0, 0, 1, 1, 1, 1);
    const v8i perm_i1 = _mm256_setr_epi32(0, 0, 1, 1, 2, 2, 3, 3);
...
void run_test_logic(int L, v8i* A, v8i* B, uint32_t* roots, uint32_t* inv_roots, int& root_size) {
    int L8 = L >> 3;
    get_root_mont(L, roots, inv_roots, root_size);
    dif_ntt(A, L, roots);
    dif_ntt(B, L, roots); // dif_ntt uses roots
...
int main() {
    // Declared large arrays on stack memory as requested.
    // WARNING: This consumes ~96MB of stack space (Arrays + Roots). 
    // Ensure stack limits are increased (e.g. ulimit -s unlimited) before running.
    alignas(32) v8i A[maxn8];
    alignas(32) v8i B[maxn8];
    alignas(32) v8i A_bak[maxn8];
    alignas(32) v8i B_bak[maxn8];

    // Moved root vectors to stack arrays
